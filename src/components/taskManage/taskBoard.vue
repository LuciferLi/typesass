<template>
    <section class="h-full min-h-0 w-full max-w-full overflow-x-auto overflow-y-hidden pb-2">
        <div class="grid h-full min-h-0 w-full min-w-[1548px] grid-cols-5 gap-3">
            <section
                ...
                <div class="grid min-h-0 flex-1 content-start gap-2 overflow-y-auto p-2">
                    <article
                        ...
                        <div class="flex items-center justify-between gap-2">
                            <ui-badge variant="outline">{{ column.label }}</ui-badge>
...
<script setup lang="ts">
    import { CheckOne, Focus, Loading, PlayOne, Plus, Time, ToBottomOne } from '@icon-park/vue-next';
    ...
    const columns: { status: SessionTaskStatusType; label: string; icon: Component }[] = [
        ...
        { status: 'queued', label: '排队中', icon: ToBottomOne },
     ...
     * 参数：status 为看板列状态。
     * 返回：该状态下的任务列表。
     * 边界：失败和取消任务不单独成列，仍通过按钮允许重新进入排队。
     */
    function taskListByStatus(status: SessionTaskStatusType): SessionTaskModel[] {
        if (status === 'created') {
            return props.tasks.filter(
                (task) => task.status === 'created' || task.status === 'failed' || task.status === 'cancelled'
            );
        }
